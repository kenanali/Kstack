...
description: Full competitive pricing intelligence analysis. Use when a user wants to benchmark competitor pricing, analyze tier/packaging architecture, identify pricing white space, or build a pricing report with buyer willingness-to-pay analysis. Trigger on phrases like "analyze competitor pricing", "what should we charge", "how are competitors structured", "tier analysis", "willingness to pay", or any competitive pricing research request. Also trigger when preparing a go-to-market strategy, pricing a new product, or repositioning an existing offering. Handles three input paths: market data only, persona data only, or both combined. Always produces a recommendations section with every claim tagged as [MARKET], [PERSONA], or [INFERRED].
---
