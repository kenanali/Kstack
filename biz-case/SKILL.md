...
Label any benchmark found via web search as `[BENCHMARKED — SOURCE: [Publication/Site](URL) — [date]]`. Always include the actual URL from the search result — do not use generic "Web search" labels. If the URL is not retrievable, label as `[BENCHMARKED — SOURCE: [publication name, date] — URL unavailable]`. Do not fabricate benchmark numbers — if a search returns no credible source, mark the estimate as [ASSUMPTION].
